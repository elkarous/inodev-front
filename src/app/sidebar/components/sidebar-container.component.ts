...
@Component({
  ...
  animations: [
    trigger('slideExpanded', [
      state('visible', style({ width: '*', display: 'block', opacity: 1 })),
      state('hidden', style({ width: '0px', display: 'none', opacity: 0 })),
      transition('hidden <=> visible', animate('200ms ease'))
    ]),
    trigger('slideCollapsed', [
      state('visible', style({ width: '0px', display: 'none', opacity: 0 })),
      state('hidden', style({ width: '*', display: 'block', opacity: 1})),
      transition('hidden <=> visible', animate('200ms ease'))
    ]),
  ],
})
export class SidebarContainerComponent implements OnInit, DoCheck {
  @ContentChild(SidebarExpandedComponent, {static: false}) left: SidebarExpandedComponent;
  @ContentChild(SidebarMainComponent, {static: false}) main: SidebarMainComponent;
